...
# -*- coding: utf-8 -*-
"""
Created on Mon Jan 27 12:58:01 2020

@author: shariba
"""
'''
 Disclaimer: We have written all in one file so that you can debug it easily!!! Please do report if you find a bug!!!
 Note: The average of the evaluation metric scores will be used to calculate the final score 
'''
...
def get_args():
    ...
    parser.add_argument("--GT_maskDIR", type=str, default="./masks/GT/", help="ground truth mask image (5 channel tif image only)")
    parser.add_argument("--Eval_maskDIR", type=str, default="./masks/predict/", help="predicted mask image (5 channel tif image only)")
    parser.add_argument("--Result_dir", type=str, default="results", help="predicted mask image (5 channel tif image only)")
    parser.add_argument("--jsonFileName", type=str, default='metrics_semantic_endoCV2020.json', help="predicted mask image (5 channel tif image only)")
    parser.add_argument("--svgFigPlotName", type=str, default='bboxclasses_seg.svg', help="predicted mask image (5 channel tif image only)")
...
#    https://scikit-learn.org/stable/modules/model_evaluation.html
    ...
    # uncomment this for EAD challenges
#    classTypes=['Instrument', 'Specularity', 'Artefact' , 'Bubbles', 'Saturation'] 
    classTypes = ['BE', 'suspicious', 'HGD' , 'cancer', 'polyp']
    args=get_args()
    ext=['*.tif']
    epsilon = 0.0000001
    dice_valPerImage =[]
    ...
    for filename in sorted(glob.glob(args.GT_maskDIR +ext[0], recursive = True)):
        ...
        for i in range (0, len(classTypes)):
            ...
            if ca !=[]:
                ...
                distance_perImage.append(d1+d2)
#                distance_perImage.append(d2)
